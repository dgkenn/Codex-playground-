...
- **FAIL (decisively negative or structurally closed):** Directional SPECs 2/4/5/6, R4-1 (lists only 39–42h ahead), R4-2, stacked r1s2/r1s3/r2s1
- **NOT PROMISING (structural frequency/latency fails, archived):** Expansion sports, earthquakes, commodities, crypto, FDA
- **Insufficient/inconclusive (underpowered or data-walled, not disproven):** Directional SPECs 1/3/7, illiquid r1s1/r1s2/r1s3, stacked r2s2
...
|---|---|---|---|
| **Forecast-overlay sleeve** | RESEARCH-STAGE (164 paper rows / 112 settled in today's live status run; registry text still says 59 as of the 2026-07-20 build) | Informational only — no decision gate authored, no activation bar set | Author a decision layer mirroring `wx_earlylock_decision.py`'s conservative-bar pattern; a sleeve counts $0 until it passes a gate |
...
| **Weather settlement-mismatch thesis** | UNTESTED (pre-registered, coded, never executed) | No results.json exists; verifier had checked the wrong artifact (lt_spread's) | Execute fresh next session; neither a kill nor a survivor until run + adversarially verified |
| **Long-tail stale-resolution thesis** | UNTESTED (pre-registered, never executed) | joined.json does not exist; a stray unregistered KXNHLPTS pull was found instead | Execute fresh next session; the stray pull needs its own pre-registration if ever used |
...
**In order of evidence-weighted realism, the only remaining paths toward (not to) $4k/mo:**
1. **Forecast-overlay sleeve** — the only accruing signal with real volume (112 settled paper rows), but it has no decision gate yet and counts $0 until it passes one; even if it cleared the deployed sleeve's bar, the base scenario tops out ~$838–885+/mo across $2k–$10k bankrolls.
2. **Depth_adaptive validation** — pending the 15-distinct-day snapshot gate (currently 1/15) plus station-median stability; if it fails, the pure-bankroll path hard-caps near $1,047–$1,318/mo permanently; if it passes, proceed toward $500 sizing with alpha=0.25 against the pessimistic fire-conditional depth measure (no larger $/mo figure for a pass is honestly quantifiable today).
3. **Synoptic measured latency** — 14-day trial; the paid 1–2-min tier would cut steady-state detection from ~8 min, but WX_NEARMISS_DIAGNOSIS shows this cannot fix adverse selection: the market beat the lock rule itself by a median 106 min in 10/10 sampled cases (~0% convertible, ≈2–4% theoretical upper bound).
4. **Expansion Family 2 (mentions)** — ~$18/mo defensible, ~$140/mo speculative only if a ×8 sibling extrapolation AND a still-unmeasured transcript-feed latency both validate; requires an external caption source not yet available.
