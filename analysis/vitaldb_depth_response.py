#!/usr/bin/env python3
"""DEPTH-RESPONSE: does the pressure fall scale with HOW DEEPLY suppressed the bin is?
...
# Suppression FRACTION of the bin -- a third exposure-response axis, independent of run length and of
# cumulative occupancy. Run length asks "how long has this lasted", occupancy asks "how much of the recent
# window", and this asks "how deeply suppressed is this 30 s". If all three are graded, the dose-response is
# not an artefact of one particular way of counting.
BANDS = [(0.001, 0.10, "1-10 % of bin"), (0.10, 0.25, "10-25 %"), (0.25, 0.50, "25-50 %"),
         (0.50, 0.80, "50-80 %"), (0.80, 1.01, "80-100 %")]
...
def report(D, mask, title, mode="run"):
    ...
    if n < 8000:
        ...
        return
    keys = ("case", "run", "occ", "frac", "m0", "dz", "dce", "pre", "df", "db")
    ...
    span = ends - starts
    if mode == "run":
        expo = [((sub["frac"] >= lo) & (sub["frac"] < hi)).astype(float) for lo, hi, _ in BANDS]
