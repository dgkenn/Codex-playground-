...
- Requirement mortality OR per SD: **3.796 (age-adjusted) -> 3.118 (fully adjusted)**, CI [2.83, 3.462]; **attenuation 24.3%**.
- AUC: severity-only 0.709 -> +requirement 0.78 (**ΔAUC 0.0706**).
- Severity covariates: ['age', 'n_vasopressors', 'comorbidity_count'].

## Verdict
Requirement->mortality OR 3.796428813966017 (age-adj) -> 3.1178302508286153 (FULLY adjusted for n-vasopressors + comorbidity + ICU-LOS + age), CI [2.83, 3.462]; attenuation 24.3%. SURVIVES severity adjustment -- the requirement carries mortality information BEYOND crude severity (adds AUC +0.0706 over the severity model). The dose is not merely a severity proxy. NOTE: no lactate/SOFA (would attenuate further); observational.
