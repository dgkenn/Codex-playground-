# HANDOFF — project state at R392 (2026-07-27)
...
reports), I-CARE (607 post-cardiac-arrest) and VitalDB (intraoperative). It has **392 logged results**. The
...
**The companion finding (R389):** the clinician slowing flag's protective effect is genuinely larger after
anoxia — interaction **−0.750 [−1.433, −0.116]** — which a generic-severity account does not predict.
...
1. **External replication is weak, and this is the headline caveat.** I-CARE AUC is **0.511 [0.464, 0.557]**
...
3. **Death-ascertainment conditioning (L3)** applies to every estimate.
4. **Aetiology comes from ICD codes**, which this project has already had to correct once, and the entire
   finding is an aetiology contrast.
...
**0. Replicate the aetiology reversal in a mixed-aetiology cohort. This is the gate on everything else.**
   TUH is in the protocol and is the obvious candidate. I-CARE cannot help. Until this exists, the lead is a
   single-cohort finding.

**1. Remove the death-ascertainment conditioning (L3)** from the reversal, or show it is unchanged when the
   conditioning is relaxed.

**2. Re-derive aetiology without ICD codes if at all possible**, or at minimum run the reversal against a
   second aetiology definition.

**3. The spindle test.** `docs/research/48_RESEARCH_LANDSCAPE.md` §B: sleep spindles are generated by the
