...
| Single-test / run-once design | 2, 12, 14 | `phase2/unlock_and_test._acquire_run_once` -- creates `artifacts/phase2/RUN_ONCE.lock` with `O_CREAT | O_EXCL`; any subsequent call raises `RunOnceViolation`; `common/config.validate` requires `phase2.run_once == True` | `tests/test_integrity.py::TestRunOnceLock.test_second_test_aborts`, `TestConfigInvariants.test_run_once_required` |
...
| Cross-site reproducibility ARI gate | 6, 9 | `analysis/cluster.cross_site_reproducibility` -- applies the frozen assigner to a held site and compares against an independently re-derived clustering; returns `{"pass": ari >= reproducibility_ari_threshold}`; threshold from `config.yaml::sites.reproducibility_ari_threshold` | No dedicated unit test (requires held-out data); structural test via `TestAssignerFreeze.test_assigner_hash_and_assign` |
| Negative controls must fail (structure must collapse under surrogates; association must collapse under shuffled outcome) | 11, 13 | `analysis/audits.surrogate_structure_collapse` -- real clustering-quality score must exceed the 95th percentile of phase-randomized surrogate scores; `analysis/audits.negative_control_shuffled_outcome` -- phenotype<->outcome MI must collapse under permuted outcome | `tests/test_analysis.py::TestNegativeControls.test_surrogate_structure_collapse` |
| Frozen-manifest hash verified before unlock | 3 | `phase2/unlock_and_test.run` -- loads the manifest with `phase2/freeze.load_manifest`, compares `frozen_pipeline_hash` to the caller-supplied `expected_pipeline_hash`, raises `FirewallBreach` on mismatch before calling `guard.unlock_held_out` | `tests/test_integrity.py::TestFreezeManifest.test_build_and_write_manifest` |
