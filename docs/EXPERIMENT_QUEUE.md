...
|---|---|---|
| ~~1~~ | ~~Waveform shape rather than band power~~ | **DONE — R412, and the premise was wrong.** Stereotypy contributes **+0.0088** to the flag coefficient; **burst amplitude** contributes **+0.0783**, and amplitude is a scale parameter, not shape. Total attenuation **+0.1186 [+0.0501, +0.1998]**, outside a permutation placebo of [−0.0228, +0.0136], and not a between-site gain artefact. **89 % of the residual survives.** |
| **1** | **The flag residual is now a STANDING NEGATIVE — find a fifth candidate** | All four named candidates are closed: background spectrum (B3), topography (T3), shape/stereotypy (R412), reactivity (unavailable in this schema). Burst amplitude is a real but *partial* contributor that is near-orthogonal to burden (r = −0.0001) and intra-burst content (r = −0.023) — which is a clue about what the missing quantity looks like, not an answer. **A finding that retrodicts this negative is worth more than one that adds a positive.** |
