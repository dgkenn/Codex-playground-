...
def main():
    ...
    assets = (sys.argv[3].split(",") if len(sys.argv) > 3 else ["btc", "eth", "sol", "xrp"])
    tenors = ([int(x) for x in sys.argv[4].split(",")] if len(sys.argv) > 4 else [5, 15])
    ...
    bt.start(); tt.start()
    print(f"copy v2 | {WALLET[:10]} | depth={depth}tk assets={assets} tenors={tenors} | {dur}s", flush=True)

    def book_at(tok, ts):
        with lock:
            snaps = list(books.get(tok, ()))
        cand = [x for x in snaps if x[0] <= ts + 3]
        best = max(cand, key=lambda x: x[0]) if cand else (min(snaps, key=lambda x: abs(x[0] - ts)) if snaps else None)
    ...
    while time.time() < t_end:
        ...
        for t in batch:
            ...
            rec = {"ts": int(t["timestamp"]), "tok": tok[:8], "side": side, "price": p}
            if tok not in idx:
                miss["market_not_tracked"] += 1; rec["why"] = "market_not_tracked"
            elif snap is None:
                miss["no_book"] += 1; rec["why"] = "no_book"
            else:
                _, bb, ba = snap; tick = idx[tok][2]; rec["bid"] = bb; rec["ask"] = ba
                hit = []
                for d in DEPTHS:
                    if side == "BUY":
