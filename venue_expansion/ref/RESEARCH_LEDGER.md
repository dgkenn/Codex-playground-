...
| 35 | Sports/forecasting-axis: KXNFLGAME vs sharp Vegas closing line AND a walk-forward Elo trained model | FORECAST_MODEL.md | CONFIRMED NULL (sharp-line) + CONFIRMED NEGATIVE (trained model) | n=274/282 games (2025 season) with a real pregame (last-minute-before-kickoff, candlestick-API) crossing price. Sharp-line: EV/contract **−0.0001**, day-clustered t=−0.13, n=189 trades/49 gamedays at the loosest threshold; only 3/274 games ever show a ≥3c Vegas-vs-Kalshi gap. Trained model (Elo, fit 1999–2024, true walk-forward OOS 2025, Brier 0.236): EV **−0.043 to −0.067**/contract, t=−1.33 to **−2.21**, getting *more* negative as model/market disagreement grows — the market beats the mechanical model, not the reverse | Pure market efficiency, not an execution/capacity kill (KXNFLGAME is liquid, ~1c spreads, median volume in the millions/season): Kalshi's pregame moneyline price matches the devigged Vegas closing consensus to within a few tenths of a cent on the median game (spot-checked exactly on a real settled game, DAL@PHI 2025-09-04: devigged Vegas 22.32% vs Kalshi 21–22c). Closes the "forecast vs market price" axis for sports the same way `FORECAST_OVERLAY_BACKTEST.md` closed it for weather |
| 36 | Latency axis: crypto Up/Down (KXBTC/KXETH hourly) speed-vs-EV, model-vs-print at L=1s..300s | LATENCY_EDGE.md | NO-EDGE-ANY-LATENCY (CONFIRMED, two independent builds + Fable adversarial verify) | 6-point L grid (1s/5s/15s/30s/60s/300s), fee-inclusive real next-print fills. Curve is **flat**, not decaying, across the whole grid — no L reaches the pre-registered \|t\|≥3 bar. Primary L=1s: n=11,359 fills, mean-of-market-means +1.41c (t=0.116) but fill-weighted per-contract mean **−1.7c**; independent curve2 rebuild: **−9.5c** (t=0.12). Day-clustered (corrected 8 distinct calendar days, not 11 as primary claimed — BTC/ETH share Nov 20/21/22): t=0.12–0.18, null unchanged | Reference "Kalshi price" is the last trade print, not a live quote — the raw model-vs-print gap is substantially print staleness (curve2's 59-60% directional accuracy fully absorbed by real fill time), same stale-print artifact family as #19/#20/#34. Flat (not decaying) curve means the information is priced in before the fastest measurable latency (1s); gives no basis to extrapolate a sub-100ms/co-location edge. Fills are next-print proxies (no order book/spread paid) so measured EV is an upper bound — real taker EV is at/below the negative point estimates. **Closes the "Short-term crypto Up/Down temporal-arb" item parked in §4 pending exactly this measurement.** Capacity $0/mo, no infra shipped |
| 37 | Weather ensemble-probability info-gap: 51-member ECMWF ensemble P(clear strike) vs Kalshi crossing price, dayahead + samemorning leads | WEATHER_ENSEMBLE.md | CONFIRMED NULL (Fable adversarial verify) | Backtest A (true ensemble) NEVER COMPLETED — `bt_ens/bt_ens.py` has no output; recon proved Open-Meteo's `ensemble-api` historical path is all-NULL beyond ~92h back for every model tested, a hard retention wall (house note "historical ensemble confirmed working" REFUTED and corrected). Backtest B (substitute fitted-Gaussian on bias-corrected forecast, 4 cities KDEN/KMIA/KSEA/KPHX, HIGH series, 50-day OOS, real tape entries): dayahead −0.55c/ct (day-clustered t=−0.34, CI [−3.70,+2.60], N=756/50 days), samemorning −0.59c (t=−0.61, CI [−2.50,+1.31], N=690/50 days); market Brier (0.106/0.049) beats model Brier (0.133/0.122) at both leads | Even ensemble-grade uncertainty (a strictly higher bar than the dead point-forecast overlay, #FORECAST_OVERLAY) is already priced by Kalshi weather; the fitted-Gaussian substitute is measurably overconfident in mid/upper probability bins, so this closes THIS model's edge, not any conceivable true-ensemble edge. samemorning arm has a look-ahead flaw (day0 values can come from runs issued after the 09:00 entry) but it biases *toward* finding an edge and the result is still null, strengthening rather than undermining the conclusion. Capacity $0/mo; no sleeve built |
...
- **NULL (indistinguishable from zero after correction):** Early-lock (historical), directional funnel as a whole, weather ensemble-probability info-gap (higher bar than the dead point-forecast overlay, still null)
...
- **NULL / NEGATIVE vs a sharp external signal:** Sports forecasting axis — KXNFLGAME vs devigged Vegas closing line (null) and vs a walk-forward Elo trained model (negative, market beats the model)
- **NO-EDGE-ANY-LATENCY (speed axis quantified and closed):** Crypto Up/Down (KXBTC/KXETH) model-vs-print speed-vs-EV curve, flat and non-positive across 1s–300s — faster execution does not help; see `LATENCY_EDGE.md`
...
| **Short-term crypto Up/Down "temporal-arb + directional" (external lead)** | ❌ NO-EDGE-ANY-LATENCY (2026-07-22, `LATENCY_EDGE.md`) — NOW CLOSED | The parked prior (latency + zero-fee wall) is now **measured, not inferred**: model-vs-print speed-vs-EV curve across L=1s..300s is flat and non-positive at every point (fill-weighted EV −1.7c to −9.5c/ct across two independent builds), no L reaches \|t\|≥3. A flat (non-decaying) curve means information is priced into the fill before the fastest latency this design can measure — there is no extrapolable sub-100ms/co-location edge either, since there's no decay to project. Fable-verified: independent reproduction exact, look-ahead-clean causal variant materially identical, fee/fill mechanics reproduce the raw tape exactly | CLOSED — do not build a latency/WebSocket bot; option 1 answered NO. `p4k_params.json` untouched (no sleeve, $0/mo capacity) |
...
**Update 2026-07-22 (sports forecasting axis closed, `FORECAST_MODEL.md`): the "sharp external forecast vs Kalshi price" axis is now closed for sports the same way it was for weather.** `FORECAST_OVERLAY_BACKTEST.md` found the weather-forecast-vs-market axis dead (public forecasts are priced). This study asks the equivalent sports question — does Kalshi's `KXNFLGAME` moneyline market misprice relative to the devigged Vegas **closing** line (a genuinely sharp, professional consensus, not a public forecast) — and additionally builds a second, independent test: a walk-forward Elo model trained on 26 seasons (1999–2024) of nflverse history, continued OOS through the full 2025 season with no leakage. Both tests are null-or-negative on a well-powered, day-clustered, real-crossing-price, fee-inclusive sample (n=274/282 games, 2025 season): the sharp-line test is a clean null (EV −0.0001/contract, t=−0.13, n=189/49 gamedays), and the trained-model test is **negative and gets more negative** as the model's disagreement with Kalshi grows (EV −0.043→−0.067/contract, t=−1.33→−2.21) — direct evidence the market outperforms a mechanical, information-poor predictor rather than lagging it. Spot-checked exactly against a real settled game (DAL@PHI 2025-09-04: devigged Vegas 22.32% vs Kalshi 21–22c pregame). Capacity **$0/mo**; no sleeve built, no `p4k_params.json` change, no live-path touch.

**Update 2026-07-22 (latency axis quantified and closed, `LATENCY_EDGE.md`): the parked "short-term crypto Up/Down temporal-arb" external lead is now measured, not just inferred, and the answer is NO-EDGE-ANY-LATENCY.** Two independent backtests of a model-vs-Kalshi-print speed-vs-EV curve on KXBTC/KXETH hourly Up/Down markets, across a six-point latency grid from 1s to 300s with fee-inclusive real next-print fills, agree: the curve is **flat, not decaying**, and non-positive at every point tested (fill-weighted EV −1.7c/ct at L=1s in the primary, −9.5c/ct in the independent rebuild; no L reaches the pre-registered \|t\|≥3 bar). Fable adversarial verification reproduced the primary exactly, confirmed a strictly-causal (no-look-ahead) variant is materially identical, and corrected a flattering mean-of-market-means framing to the honest fill-weighted number — the null held and *strengthened* under both corrections. A flat curve is decisive here: it means the market has already priced the signal into the fill by the fastest latency this design can measure, so there is no decay to extrapolate toward a sub-100ms or co-located edge either. **This answers option 1 of the operator's constraint-relaxation menu (§ below): faster/co-located execution is not worth building for this strategy class.** Capacity $0/mo, no infrastructure shipped, live path untouched.
...
**META-CONCLUSION (final, 2026-07-22):** every strategy class a retail, public-data, minutes-cadence bot can reach has now been tested on real data at tradeable prices across BOTH major prediction-market venues (Kalshi: universe→category→series, taker+maker; Polymarket: zero-fee, 1,447 markets), all adversarially verified. No deployable non-latency edge exists. The rigorous, exhaustively-mapped negative result is the deliverable. **The latency-relaxation avenue has since been measured and closed too (`LATENCY_EDGE.md`, 2026-07-22): a flat, non-decaying speed-vs-EV curve on KXBTC/KXETH Up/Down across 1s–300s means faster/co-located execution does not unlock an edge here — it was the one operator-controlled constraint that looked plausibly worth relaxing, and the data says no.** What remains is a domain forecasting model (different project, efficient-where-liquid), or accepting the validated mechanical sleeve (~$150/mo conservative_live) and not depositing past the canary.
...
**FINAL STATUS (pre-ensemble):** four avenues now closed — single-venue Kalshi (all granularities, taker+maker), single-venue Polymarket (1,447 markets, tradeable prices), cross-venue arbitrage, and the latency/execution-speed axis (`LATENCY_EDGE.md`: flat, non-decaying, non-positive speed-vs-EV curve on crypto Up/Down across 1s–300s, no edge at any tested or extrapolable latency). No deployable non-latency edge exists for a retail bot on either major prediction-market venue or between them, and building faster execution does not create one either. Search exhausted; remaining moves are a domain forecasting model (different project) or accepting the ~$150/mo validated mechanical sleeve.

## 9. WEATHER ENSEMBLE-PROBABILITY THESIS TESTED (2026-07-23, WEATHER_ENSEMBLE.md, Sonnet workers / Fable judge)

**DEPLOYABLE: NO.** The forecast-vs-Kalshi-weather axis was closed at the deterministic
point-forecast level by `FORECAST_OVERLAY_BACKTEST.md` (look-ahead + settle bug; honest
edge −0.016c/ct, t=−1.74). This round asked the harder question: does a genuine
**ensemble-probability** distribution — the info-gap between what a 50-member operational
ensemble knows and what a retail point-forecast crowd prices — beat the market once the
overlay's two known bugs are fixed by construction? **No.** Backtest A (true 51-member
ECMWF ensemble) never produced a numeric result: Open-Meteo's `ensemble-api` historical path
is all-NULL beyond ~92 hours back for every ensemble model tested, a hard data-retention
wall that **refutes and corrects** the prior house note claiming historical ensemble access
was confirmed working. Backtest B substituted a fitted-Gaussian (bias-corrected forecast ±
FIT-window sigma) and ran a full day-clustered OOS test (4 cities, HIGH series, 50 days,
real tape entries, house fee formula, both-sides settlement): null at both leads tested
(dayahead −0.55c/ct t=−0.34; samemorning −0.59c/ct t=−0.61), with the market's own crossing
price beating the model on Brier score at both leads. Fable adversarial verification
confirmed the numbers reproduce, flagged one look-ahead flaw in the samemorning arm (biases
*toward* an edge, so the null survives a fortiori), and confirmed the other four house kill
risks (real tape entry, fee formula, both-sides settlement, day-clustering) are clean.
Capacity **$0/mo**; no sleeve built (`wx_ens_model.py`/`wx_ens_paper.py`/
`wx_ens_decision.py` were the task's contingency deliverables for a deployable result and
were not needed); no `p4k_params.json` change; live path untouched. **The weather forecasting
axis is now closed at both the point-forecast and the ensemble-probability level: Kalshi
weather prices ensemble-grade uncertainty, not just a point estimate, as well or better than
