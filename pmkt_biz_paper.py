#!/usr/bin/env python3
"""pmkt_biz_paper.py -- FORWARD paper sleeve #3: Polymarket BUSINESS/COMPANIES longshot premium
...
POS = "pmkt_biz_positions.jsonl"
SET = "pmkt_biz_settled.jsonl"
BAND = (0.10, 0.35)
# recurring macro-release families (the edge's population). NOT long-dated one-offs.
QUERIES = ["earnings", "IPO", "acquisition", "merger", "revenue", "will .* stock",
           "CEO", "layoffs", "bankruptcy", "market cap", "quarterly", "guidance"]
    ...
    with open(POS, "a") as fh:
        for q in QUERIES:
            ...
            for ev in d.get("events", []):
                ...
                seen_events.add(et)
                mkts = ev.get("markets", [])
                if len(mkts) < 1:            # business events are often single binaries (IPO/earnings/M&A)
                    continue
                ...
                for m in mkts:
                    ...
                    have.add(mid_)
                    n += 1
    print(f"[biz snapshot] {len(seen_events)} macro-release events scanned, {n} new bucket-longshot paper-sells")
...
def settle():
    ...
    with open(SET, "a") as fh:
        for p in openp:
            ...
            n += 1
    print(f"[biz settle] {len(openp)} open checked, {n} newly settled")


def report():
    s = _load(SET)
    if not s:
        print("[biz report] no settled econ paper positions yet — CLOCK-NOT-STARTED")
        return
    ...
    print(f"[biz report] PMKT-BIZ forward paper gate: status={status}")
