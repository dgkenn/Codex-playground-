"""Submental EMG on EXACTLY the windows the five-stage EEG table used -- the control E69 needs.
...
OUT = os.path.join(HERE, "..", "results", "sleep_edfx_emg.csv")
FIELDS = ["recording_id", "subject", "label", "emg_mean", "emg_median", "emg_p90", "emg_sd",
          "n_samples", "sfreq", "channels"]
    ...
    with open(out_path, "a", newline="") as fh:
        ...
        for i, r in enumerate(todo, 1):
            try:
                ...
                x = x[np.isfinite(x)]
                if x.size < 5:
                    raise ValueError(f"only {x.size} finite EMG samples")
                w.writerow({"recording_id": r["recording_id"], "subject": r["subject"],
                            "label": r["label"], "emg_mean": f"{float(np.mean(x)):.6g}",
                            "emg_median": f"{float(np.median(x)):.6g}",
                            "emg_p90": f"{float(np.quantile(x, 0.90)):.6g}",
                            "emg_sd": f"{float(np.std(x)):.6g}",
                            "n_samples": int(x.size), "sfreq": f"{float(sf):.4g}",
                            "channels": "|".join(ch)})
                n_ok += 1
