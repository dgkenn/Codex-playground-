"""Compute THIS repo's `sync_fast_slow` on DOSE-I raw EEG, beside the depositors' own SFS and PFS.
...
from bsde.features.bis_subparams import sync_fast_slow                    # noqa: E402
...
OUT = os.path.join(HERE, "..", "results", "dosei_sfs_check.csv")
...
FIELDS = ["recording", "t_s", "mine_sfs", "their_sfs", "their_pfs", "soc", "moaas"]
...
def peeg_series(z: zipfile.ZipFile, rec: str):
    ...
    return {_ts(r["Time"]): (_f(r["SFS"]), _f(r["PFS"]), _f(r["SOC"]), _f(r["MOAAS"]))
            for r in rows}
        ...
        for i, rec in enumerate([r for r in recs if r not in done], 1):
            ...
            for ts_abs in sorted(peeg):
                ...
                w.writerow({"recording": rec, "t_s": f"{t:.0f}",
                            "mine_sfs": f"{sync_fast_slow(seg, SFREQ, nfft=NFFT):.10g}",
                            "their_sfs": f"{s:.10g}", "their_pfs": f"{p:.10g}",
