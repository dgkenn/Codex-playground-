#!/usr/bin/env python3
"""E244 -- which measures transport a THRESHOLD, now that the distributional ratio is known not to matter?

PRE-REGISTRATION. Written and committed before the numbers below this line exist.
SUCCESSOR OF E243 by an instrument change: the estimand becomes threshold transportability, applied to
...
statistic must be shown to disagree with -- otherwise the replacement is cosmetic (rule 60).
    ...
    python bsde/src/bsde/experiments/e244_threshold_transport.py
"""
...
OUT = "bsde/results/e244_threshold_transport.json"
MIN_SUBJECTS = 10
MIN_MEASURES = 8
GAP_OK = 0.05
N_PLACEBO = 500
    ...
    for c in shared:
        ...
        for lab, (src, tgt) in (("sleep_edfx->ds006695", (A, B)), ("ds006695->sleep_edfx", (B, A))):
            ...
            if ceil <= 0.55:
                ...
                continue
            rand = np.asarray([apply_threshold(tw, tn, float(rng.uniform(min(sw.min(), sn.min()),
                                                                        max(sw.max(), sn.max()))), sign)
                               for _ in range(N_PLACEBO)], float)
            res[c][lab] = {"source_acc": sacc, "threshold": thr, "sign": sign,
                           ...
                           "beats_placebo": bool(float(np.mean(rand >= acc)) < 0.05)}
    print()
