#!/usr/bin/env python3
"""E241 -- does propofol's peak-shift null survive when both arms are read at MATCHED peak prominence?
...
(rule 59).
    ...
    python bsde/src/bsde/experiments/e241_prominence_matched_peak.py
"""
...
OUT = "bsde/results/e241_prominence_matched_peak.json"
...
def main() -> int:
    ...
    N = int(max(MIN_WINDOWS, np.percentile(counts, 25) // 1))
    print(f"usable windows per case: median {np.median(counts):.0f}, 25th pct {np.percentile(counts, 25):.0f}"
          f" -> retaining the top N = {N} by prominence per case")
        ...
        for a in arms:
            ...
            for c in arms[a]:
                ...
                if len(rows) < MIN_WINDOWS:
                    continue
                if restrict:
                    rows = sorted(rows, key=lambda r: -_num(r, "prominence"))[:N]
                    if len(rows) < MIN_WINDOWS:
                        continue
                    rows = sorted(rows, key=lambda r: _num(r, "meta_t_s"))
    ...
    unres, res = build(False), build(True)
    ...
    # ---- G4: does the restriction bite? --------------------------------------------------------------
    keep, drop = [], []
    for a in arms:
        for c in res[a][0]:
            allp = sorted([_num(r, "prominence") for r in by[c] if np.isfinite(_num(r, "prominence"))],
                          reverse=True)
            if len(allp) > N:
                keep.append(float(np.median(allp[:N])))
                drop.append(float(np.median(allp[N:])))
    bite = float(np.median(keep) - np.median(drop)) if keep else float("nan")
    g4 = np.isfinite(bite) and bite > abs(g_un)
    print(f"G4 the restriction bites: retained median {np.median(keep):.4f} against discarded "
          f"{np.median(drop):.4f}, margin {bite:+.4f} vs the arm gap {abs(g_un):.4f} "
    ...
    with open(OUT, "w") as fh:
        json.dump({"N_retained": N, "unrestricted": ev_un, "restricted": ev_re,
                   ...
                   "restriction_bite": bite, "capability": cap,
