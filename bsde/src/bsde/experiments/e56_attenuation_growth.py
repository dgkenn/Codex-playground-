...
def main() -> int:
    ...
    # bootstrap the ratio r_k / r_45 over subjects
    idx_all = np.arange(len(subs))
    for k in KS:
        if k == FULL_K:
            res[k]["obs_ratio"], res[k]["ci"] = 1.0, [1.0, 1.0]
            continue
        d = []
        r2 = np.random.default_rng(SEED + k)
        for _ in range(REPS):
            i = r2.choice(idx_all, size=idx_all.size, replace=True)
            ss = [subs[j] for j in i]
            vk = np.array([labels[k][s] for s in ss], float)
            vf = np.array([labels[FULL_K][s] for s in ss], float)
            mm = np.array([mk[s][MARKER] for s in ss], float)
            o = np.isfinite(vk) & np.isfinite(vf) & np.isfinite(mm)
            a, b = spearman(vk[o], mm[o]), spearman(vf[o], mm[o])
            if math.isfinite(a) and math.isfinite(b) and abs(b) > 1e-6:
                d.append(a / b)
        d = np.sort(np.array(d))
        res[k]["obs_ratio"] = res[k]["r_marker"] / base if base else float("nan")
        res[k]["ci"] = [float(np.quantile(d, .025)), float(np.quantile(d, .975))] if d.size else [float("nan")] * 2

    print("\n   observed vs predicted ratios (r_k / r_45):")
    for k in KS:
        if k == FULL_K:
            continue
        lo, hi = res[k]["ci"]
        inside = math.isfinite(lo) and lo <= res[k]["pred_ratio"] <= hi
        print(f"      k={k:3d}  predicted {res[k]['pred_ratio']:.3f}   observed {res[k]['obs_ratio']:+.3f} "
              f"[{lo:+.3f}, {hi:+.3f}]   prediction {'INSIDE' if inside else 'OUTSIDE'} the interval")

    small, full = res[KS[0]], res[FULL_K]
    lo, hi = small["ci"]
    if abs(full["r_marker"]) < 0.05:
        verdict = ("NOT INFORMATIVE -- r at the full trial count is indistinguishable from zero, so the "
                   "ratios have no denominator.")
    elif not math.isfinite(hi) or hi >= 1.0:
        verdict = ("REFUTED -- the smallest-k correlation is not detectably below the full-k one. The "
                   "correlation does NOT respond to label precision, so the attenuation reading is wrong: "
                   "the marker is weak rather than attenuated, and better labels will not rescue "
                   "Challenge B.")
    elif lo <= small["pred_ratio"] <= hi:
        verdict = ("CONSISTENT WITH ATTENUATION -- r grows with trial count and the parameter-free "
                   "Spearman-Brown prediction sits inside the observed interval.")
    else:
        verdict = (f"GROWS, BUT NOT AS PREDICTED -- r rises with k (ratio {small['obs_ratio']:.3f} "
                   f"[{lo:.3f}, {hi:.3f}]) while Spearman-Brown predicts {small['pred_ratio']:.3f}. Label "
                   "precision matters but the classical model does not describe how much.")
    print("\n" + "-" * 100)
