#!/usr/bin/env python3
"""E232 -- the alpha reversal restricted to remifentanil-exposed cases.

PRE-REGISTRATION. Written and committed before the numbers below this line exist.
SUCCESSOR OF E229 by a COHORT RESTRICTION, declared here with the reason and the count.

WHY. E229 established that across a 15-measure panel, 10 of 11 measures clearing a donor null in both
arms agree in direction (exact binomial p = 0.0059) and exactly one reverses: `relative_alpha_power`,
+0.1189 against propofol effect-site concentration and -0.2482 against sevoflurane end-tidal. E230 and
E231 then tried to remove case-mix confounding by matching, and that line is closed by rule 58 after
two placebo failures -- but E231's BALANCE TABLE is what matters here. Remifentanil, which E230/E231
named as a confound and declared they would handle, has a standardised mean difference of 1.1041
before matching and **1.2105 after**: the match made it worse. Rule 54, committed inside files that
cite rule 54.

THE NUMBERS THAT FORCE THIS DESIGN. 94.7 percent of propofol-only cases carry remifentanil (108 of
114) against 51.1 percent of sevoflurane-only cases (45 of 88). **A remifentanil-FREE contrast would
have 6 propofol cases against 43 and cannot be run at all.** The remifentanil-EXPOSED stratum has 108
against 45. So the opioid confound is removable in exactly one direction, by RESTRICTION rather than
by matching, and this file restricts.

WHAT THAT BUYS AND WHAT IT DOES NOT. Inside the restricted stratum every case receives remifentanil,
so opioid PRESENCE cannot differ between arms. Opioid DOSE still can, and is not controlled; that is
stated as a limit rather than disclaimed away. The restriction is also not innocent in the other
direction (rule 14): it removes 43 of 88 sevoflurane cases, and those are the ones whose anaesthetist
chose volatile-without-opioid, plausibly a different case mix. Both facts are reported.

THE PLACEBO IS AN ARM-LABEL PERMUTATION THIS TIME (catalogue rule 88, written after E230 and E231 both
died on covariate placebos). Permuting the arm label destroys the drug contrast while preserving the
cohort, the covariates, the windows and the code path -- it is the only destruction that matches this
estimand, because a covariate placebo changes which subset of an arm is kept and never the arm
distinction. It is compared against its DISTRIBUTION over draws, never its mean (rule 37).

PREDICTION, REGISTERED. If the reversal is a drug effect it survives restriction: alpha still clears
the donor null on consistency in both arms, with opposite signs, and remains the ONLY feature doing
so. If it is an opioid effect it disappears, and the direction agreement rises from 10 of 11 to 11 of
11. The falsification condition is therefore explicit and the design can return either.

ORIGINAL E229 NOTE, retained because its arm predicate is inherited verbatim:
E227 re-run on the arms E227 was supposed to have had.

PRE-REGISTRATION. This file exists to repair a COHORT MISCLASSIFICATION, and the repair is a bug fix
rather than a design change: E224, E225 and E227 all registered "mutually exclusive arms" and all three
implemented that by asking whether a VitalDB track KEY WAS PRESENT. An anaesthesia machine logs a
sevoflurane and a desflurane channel whether or not the vaporiser is ever opened, so a propofol case on
a machine that reports gas concentrations carries an `EXP_SEVO` track that reads 0.00 from end to end.
Measured directly: of 101 cases excluded as "both techniques", most have a volatile track whose maximum
value over the entire record is exactly zero. That is catalogue rule 6 -- check that a column is
POPULATED before designing around it -- and one pass of `max(v) > 0` over 250 cases would have caught it.

THE COST, MEASURED. Classifying by whether a track is EVER NONZERO instead of whether its key exists:

    propofol-only     44  ->  114 cases
    sevoflurane-only  90  ->   88 cases
    genuinely both   101  ->   31 cases
    desflurane-only    -  ->   14 cases

The propofol arm was run on 44 of 114 available cases. The sevoflurane arm is essentially unchanged,
so the correction is one-sided: every propofol number this project has reported comes from 39 percent
of the eligible cases, and the 70 excluded ones were excluded on a property of the ANAESTHESIA MACHINE
rather than of the patient or the drug. Rule 14 requires exclusions to be reported and checked for
outcome-relatedness; a machine that reports gas is plausibly a different theatre and a different case
mix, so this exclusion cannot be assumed innocent and the corrected arm is the one that must be read.

NOTHING ELSE CHANGES. The statistic, the gates, the donor placebo, the capability controls, the
thresholds and the verdict rule are E227's verbatim. Only the arm predicate moves, from key presence
to `max(value) > 0`.

ORIGINAL E227 QUESTION, unchanged: is the propofol arm's weak coupling ABSENCE of coupling, or coupling
with a patient-specific SIGN?

PRE-REGISTRATION. Written and committed before the numbers below this line exist.
SUCCESSOR OF E227. Instrument changed; cohort, arms, thresholds and horizon are identical to E225's.
...
exposures E224 used, so the two files are directly comparable.
    ...
    python bsde/src/bsde/experiments/e232_remifentanil_restricted.py
"""
...
OUT = "bsde/results/e232_remifentanil_restricted.json"
...
def main() -> int:
    ...
    def live(tr, key):
        """A track counts as an agent only if it is EVER NONZERO.

        The whole point of this file. A machine logs its gas channels whether or not the vaporiser is
        opened, so `key in tr` is a fact about the monitor and not about the anaesthetic (rule 6).
        """
        if key not in tr:
            return False
    ...
    for c in by:
        tr = tracks[c]["tracks"]
        hp = live(tr, "Orchestra/PPF20_CE")
        hs = live(tr, "Primus/EXP_SEVO")
        hd = live(tr, "Primus/EXP_DES")
        if any(k in tr for k in VOL) and not (hs or hd):
            misclassified += 1
        if not live(tr, "Orchestra/RFTN20_CE"):
            no_remi += 1
            continue                      # RESTRICTION: remifentanil-exposed cases only
        if hp and (hs or hd):
            continue
        if hp:
    ...
    print("arms:", {k: len(v) for k, v in arms.items()},
          f"| {no_remi} cases EXCLUDED for carrying no remifentanil (rule 14: this removes "
          "43 of 88 sevoflurane cases, whose anaesthetist chose volatile-without-opioid)")
