...
def main() -> int:
    ...
    for _ in range(400):
        ...
        for s in sess:
            y = s["result"].copy()
            fin = np.flatnonzero(np.isfinite(y))
            y[fin] = y[fin][grng.permutation(fin.size)]
            tmp.append({**s, "result": y})
        v = _pos_resid_mean(tmp)
        if np.isfinite(v):
            g2_null.append(v)
    gn = np.asarray(g2_null)
    lo, hi = float(np.quantile(gn, 0.025)), float(np.quantile(gn, 0.975))
    res["G2_position_residual_r"] = g2
    res["G2_null"] = {"mean": float(gn.mean()), "lo": lo, "hi": hi, "n": int(gn.size)}
    res["G2_pass"] = bool(np.isfinite(g2) and lo <= g2 <= hi)
    print(f"   G2 position removed: mean r(index, residual outcome) = {g2:+.4f}   "
          f"vs measured null [{lo:+.4f}, {hi:+.4f}] over {gn.size} within-session permutations   "
