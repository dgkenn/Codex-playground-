#!/usr/bin/env python3
"""E225 -- is the propofol arm's weak coupling ABSENCE of coupling, or coupling with a patient-specific SIGN?

PRE-REGISTRATION. Written and committed before the numbers below this line exist.
SUCCESSOR OF E224. Instrument changed; cohort, arms, thresholds and horizon are identical to E224's.
...
are summarised two ways instead of one.
    ...
    C  (consistency)  = |median across cases of the SIGNED rho|, per feature.
...
PRIMARIES, all per feature, all with a MEASURED donor null rather than an assumed one (rule 63).
  ...
  P1  propofol: the number of the evaluable features whose C exceeds the 95th percentile of its own donor
      null, and the mean excess C - C_donor95 with a cluster-bootstrap interval over cases.
...
exposures E224 used, so the two files are directly comparable.
    ...
    python bsde/src/bsde/experiments/e225_sign_varying_coupling.py
"""
...
OUT = "bsde/results/e225_sign_varying_coupling.json"
...
def summarise(per_case):
    ...
    return float(abs(np.median(v))), float(np.mean(np.abs(v))), len(v)
...
def main() -> int:
    ...
    for a in arms:
        ...
        for f in evaluable:
            ...
            rec[f] = {"C": C, "S": S, "n": n, "C_null95": nullC[a][f], "S_null95": nullS[a][f],
                      ...
                      "median_signed": float(np.median([v for v in real[a][f] if np.isfinite(v)]))}
        res[a] = rec
    print()
    ...
    agree = sum(1 for f in both
                if np.sign(res["propofol"][f]["median_signed"])
                == np.sign(res["sevoflurane"][f]["median_signed"]))
    print()
