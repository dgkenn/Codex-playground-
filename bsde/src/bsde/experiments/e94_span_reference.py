...
from bsde.experiments.e92_two_region_information_v2 import state_ds005620    # noqa: E402

OUT = os.path.join(RESULTS, "e94_span_reference.json")
STAGES = ("W", "N1", "N2", "N3")
...
def main() -> int:
    ...
    la, da, ea = ok(la), ok(da), ok(ea)
    res = {"gates": {}, "R_AWAKE": {}, "R_SPAN": {}}
    print(f"LEMON awake {la.size} | ds005620 anaesthetised {da.size} | eegmmidb awake {ea.size}")
    for k in STAGES + ("REM",):
        if k in stages:
            print(f"   sleep {k}: {np.isfinite(stages[k][0]).sum()}")

    # G1 disjoint
    ref_subs = set(la_s) | set(da_s)
    ...
    g2 = res["R_SPAN"]["extreme_fraction"] < EXTREME_MAX_FRAC
    g4 = (res["R_SPAN"]["stages"]["N3"]["median"] < res["R_SPAN"]["stages"]["W"]["median"]
          and res["R_AWAKE"]["stages"]["N3"]["median"] < res["R_AWAKE"]["stages"]["W"]["median"])
    res["gates"].update({"G2_pass": bool(g2), "G4_pass": bool(g4)})
    print(f"\nG2 support    R_SPAN extreme fraction {res['R_SPAN']['extreme_fraction']:.4f} "
    ...
    p1 = res["R_SPAN"]["monotone"]
    p2 = abs(res["R_SPAN"]["transport_eegmmidb"]) <= TRANSPORT_MARGIN
    print(f"\nP1 range      R_SPAN staircase monotone: {p1}   "
          f"(R_AWAKE was {res['R_AWAKE']['monotone']})")
    print(f"P2 transport  eegmmidb awake {res['R_SPAN']['transport_eegmmidb']:+.4f} "
    ...
    if p1 and not p2:
        ...
        v = ("SPAN DOES NOT FIX IT -- adding support below wakefulness does not restore the ordering, so "
             "the saturation was not a support problem and rule 62's diagnosis is wrong as stated.")
    else:
