...
| 30728772 | Rimbert, Front Hum Neurosci 2018;12:529 | **CORRECTED 2026-08-02 against the MEDLINE record (rule 42).** This row previously read "a subjective questionnaire as a BCI performance predictor", which inverts the paper. Its abstract states verbatim: *"Our results showed no significant correlation between BCI performance and the MIQ-RS scores. However, we reveal that BCI performance is correlated to habits and frequency of practicing manual activities."* The questionnaire is a **negative** result; the positive one is **manual-activity habits**. | **TESTED AND FAILED, E238, 2026-08-02.** `Manual activity` against mean online accuracy on all 87 subjects gives Spearman **-0.0841, permutation |p| = 0.4368**, against Rimbert's +0.381 on n = 35. The 80 %-power floor was declared in advance at rho = 0.2965, so this cohort had better than 95 % power for the published effect and did not find it — a non-replication, not a power failure. This row is therefore no longer a licence for anything: Dreyer has no live non-observational incumbent. |
