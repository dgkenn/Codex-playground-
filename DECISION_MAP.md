...
| F7 | coverage gaps (GHA cycle restarts) | 🟡 REAL LOSS — LIVE-CONFIRMED 2026-07-13 audit | Collector proxy said 87%; today's LIVE tape shows it directly: zero-box windows at 14:15/15:00/15:45Z on the ~46-min leg-restart cadence = 3-4 of 23 daytime slots (~15-17%) lost. At clean-window edge (+0.78c/box × 6 boxes) ≈ +$0.15-0.20/day forgone at current size. TENSION with node N: naive leg overlap fixes coverage but creates double-leg windows (see 12:00Z strand, ny=5 > max-fills-side 3 — two processes quoting the same window). Study: order_lifecycle/recon around leg boundaries → design a handoff that closes holes WITHOUT double-quoting (e.g., new leg quotes only windows opening after its start; old leg completes its last window). VPS (P3/step-2a) solves it structurally |
