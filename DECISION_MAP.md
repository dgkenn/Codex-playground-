...
| F-size | state-dependent entry sizing by (time, leg-price) — "bet bigger where pairing is likely" (operator 2026-07-13) | 🟡 REAL signal, but bounded + hard-rail (propose-only) | Two data facts. (1) FIRST fills are 97% at min0-2 (first-fill minute p10=p50=p90=2) → time-sizing applies to LATER boxes (2nd/3rd), not the first; the first box's lever is PRICE/book-state. (2) Pairing rises MONOTONE with first-leg price: 0-15c→79.2% pair/20.8% strand; 35-50c→87.4%/12.6%; 50-65c→90.3%/9.7%; 65-85c→92.3%/7.7%; 85-99c→95.5%/4.5% (spread flat 1c across all). Because strand loss (~45c) dwarfs box edge (~1c), risk-adjusted value is dominated by STRAND AVOIDANCE → the robust action is SIZE DOWN / veto the cheap-leg entries (p1<~35c: they pair <83%, strand >17% = where money bleeds), keep normal size in the reliable middle-high. CAVEATS: (a) aggressive UP-sizing at extreme prices is overstated — locked box edge THINS at the extremes (F4d), so a constant-edge EV calc inflates the high-price multipliers; real up-cap is modest (~1.5x, not the naive 3-5x); (b) strand size scales with quote size, so up-sizing adds tail risk; (c) at-fill pair prediction is only AUC 0.617 — the price band is coarse signal. This GENERALIZES the existing binary pair_gate into continuous size, and overlaps mo_size (markout sizing). DISPOSITION: hard-rail (sizing = propose-only, charter 1) + needs forward validation (2b) → build as a box_shadow/paper SIZING arm, adjudicate forward; NOT a live change |
