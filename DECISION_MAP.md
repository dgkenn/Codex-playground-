...
| F14 | FRACTIONAL partial-fill residuals ride to settlement unhedged | 🟡 REAL small EV leak (pre-existing, NOT from 2026-07-13 deploys) — study before any fix | 2026-07-13 21:00Z window settled −$0.20 despite looking paired. Root cause from the fill tape: a resting 2-lot NO maker order PARTIALLY filled for 1.52 contracts, leaving a fractional 0.48 long-YES residual. The strand detector flags at abs(net)>0.5, so 0.48 slipped UNDER the threshold — never marked stranded, never disposed, rode to settlement, window resolved DOWN, 0.48 YES @0.53 settled worthless: 5.52 paired boxes ×+0.01 = +$0.055, minus 0.48×$0.53 = −$0.25 ⇒ −$0.20 (matched the alert). MECHANISM: Kalshi allows fractional-contract fills; any partial fill creates a sub-1 residual, and residuals in [threshold_gap] escape the strand machinery. It's adverse-selected like any strand (exists because price moved). QUANTIFY FIRST (study, not an autonomous change — touches live disposal thresholds): scan winrec+fills for abs_strand in (0, 0.5], measure frequency × settle-P&L drag/day. CANDIDATE FIXES (propose-only): (a) lower the dispose/chase threshold to catch fractional residuals (e.g. abs(net)>=0.25 or >0 near close); (b) round completion quotes to avoid inviting partials; (c) a near-close sweep of any nonzero residual. Small $ (−0.20 on one window) but systematic — RenTec-style weak-drag worth measuring |
