...
| F10 | stale-quote sniping (take obviously mispriced resting orders; exploit other bots' missing backstops) | ❓ REGISTERED (2026-07-13, operator idea) — feasibility scan queued | Passive taking only (inducing other bots to malfunction = manipulation, out of bounds). Prior evidence snipers EXIST here: our own maker fills are sweep-toxic — we are currently the prey. Economics bar: taker fee ceil(0.07·p·(1−p)) ≈ 1.75c at p=0.5, so need mispricing vs BTC-spot theo > ~2c+margin. Infra reality: GHA loop reacts in ~1-3s; colocated ws bots win any <1s race, so ONLY exceedances persisting >~3s are capturable by us. STUDY (daily-cycle candidate): scan gha_data book snapshots vs theo; measure exceedance frequency × DURATION × takeable depth, split by vol regime and last-2-min (settlement approach — likeliest place for forgotten backstops). Verdict needs: capturable EV/day at our latency, day-clustered |
