...
| 4 | Late-window favorite (buy near-decided side) | ❌ not deployable (full 60-combo grid 2026-06-12) | Taker: fee+priced-in. MAKER full grid (T 0.90-0.99 x k{12,13,14} x z-gate, IS/OOS+Bonferroni): naive IS-best flips negative OOS; the 4 deep-favorite corners (T>=0.97, z>=2) that 'survive' Bonferroni (t=8-16) are a ZERO-LOSS-TAIL ARTIFACT — entry ~0.998, won 100% of ~172 OOS windows, but break-even needs >99.8% win and rule-of-three only rules out loss rates above 1.74% (vs 0.20% break-even). The sample CANNOT distinguish real edge from a steamroller trade; confirming needs ~1500 loss-free windows (~39 days). Classic pennies-in-front-of-steamroller; the high t-stat is a lie of zero in-sample variance. NOT deployable. Favorite premium is real only EARLIER in the window = t29. |
